...
- Interlocutors (in a discourse) have prior probability distributions for eventualities: e.g., the prior probability distribution of the eventuality described by “Mary is pregnant” is an interlocutor’s belief that this state holds (at the topic time) before considering some evidence (e.g., hearing a sentence that describes the eventuality).
