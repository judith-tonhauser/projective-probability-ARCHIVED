...
ggplot(t, aes(x=verb, y=response)) + 
  ...
  xlab("Predicate")
ggsave(f="graphs/boxplot-projectivity.pdf",height=4,width=6.5)

means = aggregate(response~verb+fact_type, data=t, FUN="mean")
means$YMin = means$response - aggregate(response~verb+fact_type, data=t, FUN="ci.low")$response
means$YMax = means$response + aggregate(response~verb+fact_type, data=t, FUN="ci.high")$response
means

means$verb <-factor(means$verb, levels=means[order(means$response), "verb"])

ggplot(means, aes(x=verb, y=response)) + 
